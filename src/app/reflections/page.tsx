...
export default function ReflectionsPage() {
  ...
  const fetchReflections = async () => {
    ...
    try {
      ...
      const res = await API.get(`/reflections/user/${currentUserId}`, { params });
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center">
        <Select value={filterEmotion || "all"} onValueChange={(val) => setFilterEmotion(val === "all" ? "" : val)}>
        ...
        {showForm && (
          ...
          >
            <ReflectionForm
              ...
              onCancel={() => { setShowForm(false); setEditingReflection(null); }}
            />
          </motion.div>
        )}
      </AnimatePresence>
